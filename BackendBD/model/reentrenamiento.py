...
def preparar_csv_crudo(df):
    ...
    cols_minimas = ["product_id", "created_at", "quantity_available", 
                    "quantity_on_hand", "quantity_reserved"]
    validar_columnas_requeridas(df, cols_minimas)
    ...
    #Adjuntar nuevos registros al dataset existente
    # existing_df = pd.read_csv(DATASET_FILE)
    # combined_df = pd.concat([existing_df, df[cols_existentes]]).drop_duplicates(subset=["product_id", "created_at"])
...
def crear_backup(version):
    ...
    backup_dataset = BACKUP_DIR / f"dataset_{version}.csv"
    ...
    if DATASET_FILE.exists():
        shutil.copy(DATASET_FILE, backup_dataset)
        logger.info(f"✓ Backup del dataset: {backup_dataset.name}")
    
    return backup_model, backup_scaler, backup_dataset


def restaurar_backup(backup_model, backup_scaler, backup_dataset):
    ...
    if backup_dataset and backup_dataset.exists():
        shutil.copy(backup_dataset, DATASET_FILE)
    logger.info("✓ Modelo, scaler y dataset restaurados desde backup")
...
def reentrenar_modelo(
    ...
    try:
        ...
        logger.info("\n Paso 1: Preparando datos nuevos...")
        df_nuevo_prep = preparar_csv_crudo(df_nuevo)
        logger.info(f"   ✓ {len(df_nuevo_prep)} filas preparadas")
        logger.info(f"   ✓ Rango temporal: {df_nuevo_prep['created_at'].min().date()} → {df_nuevo_prep['created_at'].max().date()}")
        ...
        # Cargar dataset antiguo y asegurar 'created_at' como datetime
        df_old = pd.read_csv(DATASET_FILE, parse_dates=["created_at"])
        # Forzar conversión (por si hay strings con formatos raros)
        df_old['created_at'] = pd.to_datetime(df_old['created_at'], errors='coerce')

        # Detectar filas con fecha inválida
        n_invalid = df_old['created_at'].isna().sum()
        if n_invalid:
            logger.warning(f" {n_invalid} filas en dataset antiguo tienen 'created_at' inválido y se marcaron como NaT")

        logger.info(f"   ✓ Dataset antiguo: {len(df_old)} filas")

        # Helper seguro para imprimir rangos (maneja NaT)
        def _fmt_range(s):
            if s.dropna().empty:
                return "N/A"
            mn = s.min()
        ...
        logger.info(f"   ✓ Rango temporal: {_fmt_range(df_old['created_at'])}")
        ...
        with mlflow.start_run(run_name=f"retraining_{version}"):
            ...
            # === ACTUALIZAR SIEMPRE EL DATASET MAESTRO (INDEPENDIENTE DE SI EL MODELO MEJORA) ===
            logger.info("\n Paso 10: Actualizando dataset maestro con nuevos datos...")
            ...
            logger.info(f"   ✓ Dataset actualizado: {len(df_total)} filas totales")
            logger.info(f"   ✓ Nuevas filas agregadas: {len(df_total) - len(df_old)}")
            
            # 11. VALIDAR MEJORA DEL MODELO
